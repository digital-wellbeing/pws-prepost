...
set.seed(5454545)
tmp_pre <- filter(dat_pre, pid %in% sample(unique(pid), 1000))
...
Next, we'll lok at `hours`. In @fig-lmm-pre-interaction-sens-hours we see that the results aren't fundamentally different using splines with less degrees of freedom, or even a linear term. 
