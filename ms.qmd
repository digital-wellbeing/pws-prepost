...
model <- bf(mood ~ 1 + post + (1 + post |p| pid) + (1 | ps)) +
  lf(sigma ~ 1 + (1 |p| pid)) +
  gaussian()
...
fit2 <- brm(
  ...
  iter = 500,
  control = list(adapt_delta = .95),
  file = "models/brm-pre-post-pid-ps-scale"
)

model <- bf(mood ~ 1 + post + (1 + post |p| pid)) +
  lf(sigma ~ 1 + (1 |p| pid)) +
  gaussian()
...
fit3 <- brm(
  ...
  iter = 500,
  control = list(adapt_delta = .95),
  file = "models/brm-pre-post-pid-scale"
)
...
library(posterior)
as_draws(fit4, variable = c("b_", "sd_", "cor_"), regex = TRUE) |> 
  summarise_draws(rhat)
pairs(fit4$fit, pars = variables(fit)[1:7])
```
