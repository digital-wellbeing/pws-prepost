...
p0 <- ggplot(
    ...
    scale_x_datetime(
        limits = c(
            ymd_hms("2022-09-19 16:00:00"),
            ymd_hms("2022-09-20 00:00:00")
        )
    ) +
...
if (file.exists(path)) {
    ...
    fit3 <- lmer(
        mood ~ hours0 + hours1 + hours2 + hours3 +
         (1 | ps) + 
         (1 + hours0 + hours1 + hours2 + hours3 | pid),
        data = tmp
    )
    write_rds(fit3, path)
}
...
change <- pred |>
    ...
    filter(ps %in% ps_filtered$ps) |> 
    summarise(change = last(yhat) - yhat[1], .by = c(pid, ps)) |>
    # average over sessions within participants
    summarise(change = median(change), .by = pid) 
change |> 
    # average over participants
    summarise(mean(change), median(change), sd(change))
```
...
Zoom in on pid p10057.
```{r}
pred2 <- filter(tmp, pid == "p10057")
...
#| code-fold: show
set.seed(6666)
tmp2 <- filter(tmp, pid %in% sample(unique(pid), 200)) |>
    filter(!is.na(mood), hours <= 10)
fit_g_by_lme4 <- gamm4(
...
# TODO: include a main effect
fit_g_fs_lme4 <- gamm4(
    mood ~ s(hours, bs = "cr", k = 5) + s(hours, pid, bs = "fs", xt = list(bs = "cr"), k = 5),
    knots = knots,
    data = tmp2,
    random = ~ (1 | pid) + (1 | ps)
)
plot(fit_g_fs_lme4$gam, select = 1)
```
