...
s <- readRDS("data/s.rds")
sraw <- readRDS("data/sraw.rds")
if (!is.na(N_SUBSET)) {
...
The model results confirmed the visual impressions described above: @tbl-avg indicates, with great certainty, that mood increased by about [`r paste0(map(h_avg_$hypothesis[3,4:5], ~number(.x, .001)), collapse = ", ")`] units (@fig-coef, "Raw"). 
...
posterior_pid_scaled <- apply(
  posterior_pid, 2, 
  function(x) x / posterior_pop$total_residual
) |> 
...
posterior_pid_sum <- bind_rows(
  ...
  "b_post1_scaled" = posterior_pid_scaled,
  .id = "name"
)
...
x <- posterior_pop |> 
  ...
  mean_qi(prop_pos) |> 
  mutate(x = str_glue("{percent(prop_pos, .1)} [{number(.lower*100, .1)}, {number(.upper*100, .1)}]")) |> 
  pull(x)
```

We then turned our attention to between-person differences in the difference in during and pre-play mood. With a sample of `r s$n_sub`, it is not surprising that the population-level contrast is credibly different from zero. We therefore asked the subsequent question: What proportion of the population from which the sample was drawn would we expect there to be an elevation in mood during play? The boxplots in @fig-coef show the distribution of player-specific contrasts estimated by the model, which indicated that `r x` of players in the population experience a positive shift in mood during play, compared to their moods during login.
...
#| fig.width: 3
#| fig.height: 1.4
#| fig.cap: "Difference in mood during play compared to mood at session start. Points and intervals indicate posterior means and 95%CIs of the population level difference; boxplots summarize the player-level estimates. Raw estimates are on the VAS scale, whereas Scaled estimates indicate contrasts divided by the total residual deviance of the model."
...
posterior_pop |> 
  ...
  scale_y_discrete(
    breaks = c("b_post1", "b_post1_scaled"),
    labels = c("Raw", "Scaled")
  ) +
  coord_cartesian(xlim = c(-0.25, 0.65)) +
  geom_vline(xintercept = 0, linewidth = .33, col = "dodgerblue") +
  geom_boxploth(
    data = posterior_pid_sum,
    aes(x = y, group = name), 
    outlier.shape = NA,
    notch = TRUE,
    size = 0.25,
...
Although hundreds of millions of individuals play video games, a medium sometimes considered psychologically damaging, no firm evidence exists on how games affect players’ moods---if at all. One reason that current evidence remains mixed is that games are likely to affect players differently depending on the time-scale of the investigation: Do games affect players’ well-being over long periods of time, or are the effects fleeting and only evidenced in near real-time to play? In this case study of a single game (PowerWash Simulator), we examined {s$n_sub} players’ moods from {s$n_obs} reports provided in-game. We compared players’ moods before play sessions (at login) to their moods during play. We find that on average, players report {h_avg_tab[3, "Estimate"]} {h_avg_tab[3, "CI"]} scale percentages greater mood during play than at the beginning of a play session, and that {x} of individuals in the population are predicted to have such positive changes in mood. We do not know whether the current results indicate a causal effect, since the hypothetical treatment of “choosing to play” occurs prior to our first measure, nor do we know whether and how our results would generalize to other games and player populations. Nevertheless, current results offer a promising glimpse of the potential of games in positively contributing to player mood, and of the potential of in-game telemetry and mood measure to study games’ effects on people at a short time scale where effects are likely to exist.
```
